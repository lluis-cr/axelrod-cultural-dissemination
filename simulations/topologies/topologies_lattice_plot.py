...
# Plot initial and final lattice states.
def plot_lattice_comparison(
    ...
    # Use one colour scale for both panels.
    all_cultures = sorted(
        set(tuple(culture) for culture in initial_features.values())
        | set(tuple(culture) for culture in final_features.values())
    )

    culture_index = {
        culture: index
        for index, culture in enumerate(all_cultures)
    }

    def make_grid(features):
        grid_labels = np.zeros((grid_x, grid_y), dtype=int)

        for node in G.nodes():
            x, y = node
            culture = tuple(features[node])
            grid_labels[x, y] = culture_index[culture]

        return grid_labels.T

    initial_grid = make_grid(initial_features)
    final_grid = make_grid(final_features)
    ...
    titles = [
        (
            "Initial state\n"
            f"{count_unique_cultures(initial_features)} unique cultures"
        ),
        (
            "Final absorbing state\n"
            f"{count_unique_cultures(final_features)} unique cultures"
        )
    ]

    for ax, grid, title in zip(
        axes,
        [initial_grid, final_grid],
        titles
    ):
        ax.imshow(
            grid,
            ...
            vmin=0,
            vmax=max(len(all_cultures) - 1, 1)
        )

        # Draw cell boundaries.
        ax.set_xticks(
            np.arange(-0.5, grid_x, 1),
            minor=True
        )
        ax.set_yticks(
            np.arange(-0.5, grid_y, 1),
            minor=True
        )
        ax.grid(
            which="minor",
    ...
    fig.suptitle(
        f"Two-dimensional lattice Axelrod simulation: seed {seed}",
        fontsize=14
    )

    fig.savefig(
        f"lattice_initial_final_seed_{seed}.pdf",
        bbox_inches="tight"
    )
    fig.savefig(
        f"lattice_initial_final_seed_{seed}.png",
        dpi=300,
        bbox_inches="tight"
    )
